...
export default function ReferenceEvidencePage() {
  ...
  useEffect(() => {
    getActivity(activityId)
      ...
      .catch(() => setActivityError('Activity not found.'));
  }, [activityId]);
  ...
  const runSearch = useCallback(async (page = 0) => {
    ...
    try {
      ...
      setSearchError('Failed to search evidence.');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <ActivityShell>
      <p className="am-breadcrumb">
        Workspace / <Link to="/activities" className="am-breadcrumb-link">Activities</Link>
        {activity && <> / <Link to={`/activities/${activityId}`} className="am-breadcrumb-link">{activity.activityName}</Link></>}
        {' / Reference Evidence'}
      </p>

      <div className="am-page-header">
        <h1 className="am-page-title">Reference Evidence</h1>
      ...
      {activityError && <p className="am-alert am-alert-error">{activityError}</p>}
      {searchError && <p className="am-alert am-alert-error">{searchError}</p>}

      <div className="se-layout">
        {/* Left: filters + target activity */}
        <form className="se-filter-panel" onSubmit={handleSearch}>
          ...
          {activity && (
            <div>
              <p className="am-section-label" style={{ marginBottom: '0.5rem' }}>Target Activity</p>
              <div className="se-target-panel">
                <div className="se-target-row">
                  <span className="se-target-key">Title</span>
                  <span className="se-target-val">{activity.activityName}</span>
                </div>
                <div className="se-target-row">
                  <span className="se-target-key">Area</span>
                  <span className="se-target-val">{formatAccreditationArea(activity.accreditationArea)}</span>
                </div>
                <div className="se-target-row">
        ...
        <div className="se-results-panel">
          <div className="se-results-header">
            <p className="am-section-label">
              Repository Results{' '}
              {totalElements > 0 && (
                <span className="se-results-count">
                  {totalElements} total
                  {selected ? ' · 1 selected' : ''}
                </span>
              )}
          ...
          <table className="se-table">
            <thead>
              <tr>
                <th className="se-th se-th-check"></th>
            ...
            <tbody>
              {!loading && results.length === 0 && (
                <tr>
                  <td className="am-empty" colSpan={7}>
                    No results — try adjusting your filters or{' '}
                    <Link to={`/activities/${activityId}`}>upload new evidence</Link>.
                  </td>
                </tr>
              )}
              {loading && (
              ...
              {results.map((item) => (
                <tr
                  key={item.id}
                  style={{ cursor: 'pointer', background: selected?.id === item.id ? '#f0fdf4' : undefined }}
          ...
          {/* Preview + metadata when a row is selected */}
          {preview && (
            <div className="se-preview-row">
              ...
              <div>
                ...
                <div className="se-meta-list">
                  ...
                  <div className="se-meta-row">
                    <span className="se-meta-key">Updated</span>
                    <span className="se-meta-val">{formatDate(preview.uploadedAt)}</span>
                  </div>
                  <div className="se-meta-row">
